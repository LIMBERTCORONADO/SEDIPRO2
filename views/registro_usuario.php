...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($stmt->fetch()) {
        ...
        $stmt = $conn->prepare("INSERT INTO vehiculos (chasis, tipo, clase, servicio, id_usuario, foto) VALUES (?, ?, ?, ?, ?, ?)");
        if ($stmt->execute([$chasis, $tipo, $clase, $servicio, $usuario_id, $foto])) {
            $mensaje = "✅ Usuario y vehículo registrados con éxito.";
        } else {
